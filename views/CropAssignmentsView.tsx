...
import { AlertCircle, Filter, MapPin, Edit2 } from 'lucide-react';
...
export const CropAssignmentsView: React.FC = () => {
    ...
    const handleOpenAssignment = (plotId: string, plotName: string) => {
        ...
        setEditingAssignment({
            ...
            originalStand: existing?.originalStand
        });
        setIsModalOpen(true);
    };
    ...
    const handleSaveAssignment = async (e: React.FormEvent) => {
        ...
        await Storage.savePlotAssignment(
            ...
            dataOwnerId,
            editingAssignment.originalStand,
            dataOwnerName
        );
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="Asignación de Lote">
                {editingAssignment && (
                    <form onSubmit={handleSaveAssignment} className="space-y-4">
                        ...
                        <Input
                            label="Stand Objetivo (Plantas/Ha)"
                            type="number"
                            placeholder="Ej: 60000"
                            value={editingAssignment.originalStand || ''}
                            onChange={(e) => setEditingAssignment({ ...editingAssignment, originalStand: Number(e.target.value) })}
                            helpText="Ingrese la densidad de siembra objetivo para calcular el porcentaje de logro."
                        />
