                        ...
                        {summaries.map(s => {
                            ...
                            return (
                                <tr key={s.id} onClick={() => onCardClick(s)} className="hover:bg-gray-50 dark:hover:bg-gray-700/30 cursor-pointer transition-colors">
                                    ...
                                    <td className={`px-4 py-3 text-sm italic ${getRoleColorClass(getUserRole(s.userId, data.users, dataOwnerId))}`}>{s.notes}</td>
                ...
                return (
                    ...
                    >
                        <div className="p-4 flex-1">
                            ...
                            {summary.notes && (
                                <p className={`text-sm ${getRoleColorClass(getUserRole(summary.userId, data.users, dataOwnerId))} ${getRoleBgClass(getUserRole(summary.userId, data.users, dataOwnerId))} p-2.5 rounded-lg italic border border-gray-100 dark:border-gray-700/50 mb-2`}>
                                    "{summary.notes}"
                                </p>
                            )}
                            {summary.engineerNotes && (
                                <div className={`text-xs ${getRoleColorClass('admin')} ${getRoleBgClass('admin')} p-2 rounded-lg border border-blue-100 dark:border-blue-800 mt-2`}>
